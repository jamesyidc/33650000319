...
class ABCPositionTracker:
    ...
    def fetch_okx_positions(self):
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                if data.get('success'):
                    # API返回格式: {'success': True, 'data': {'main': {...}, 'poit_main': {...}, ...}}
                    # 需要映射到 A/B/C/D 格式
                    api_data = data.get('data', {})
                    
                    # 映射account_id到A/B/C/D
                    account_id_map = {
                        'main': 'A',
                        'poit_main': 'B',
                        'fangfang12': 'C',
                    ...
                    result = {}
                    for account_id, label in account_id_map.items():
                        acc_data = api_data.get(account_id, {})
                        result[label] = {
                            'success': True,
                            'total_margin': acc_data.get('total_margin', 0),
                            'total_upl': acc_data.get('total_upl', 0),
                            'pnl_pct': acc_data.get('profit_ratio', 0),
                            'position_count': acc_data.get('position_count', 0),
                            'positions': acc_data.get('positions', [])
                        }
                        # 计算total_cost (margin作为成本)
                        result[label]['total_cost'] = acc_data.get('total_margin', 0)
                        result[label]['unrealized_pnl'] = acc_data.get('total_upl', 0)
                    
                    logger.info(f"✅ 获取持仓成功: A={result['A']['position_count']}仓, B={result['B']['position_count']}仓, C={result['C']['position_count']}仓, D={result['D']['position_count']}仓")
                    return result
    ...
    def get_position_color(self, account_id):
        ...
        # 默认阈值：20U以下无色，20-40U绿色，40-55U橙色，55U以上红色
        if total_cost == 0:
            color = 'none'
        elif total_cost < 20:
            color = 'none'
